...
export async function postApprovalLog(request: TicketRequest, approvedBy: string, approvedByUsername: string) {
  ...
  return discordApi(`/channels/${TICKET_LOG_CHANNEL_ID}/messages`, { method: "POST", body: JSON.stringify({ content: `<@&${LOGISTICS_ROLE_ID}>`, embeds: [{ title: "Ticket Log Approved", description: `<@${request.userId}> ticket log has been approved and added to the Google Staff Database.`, color: 0x57f287, fields: [
  ...
  ], image: { url: request.proof }, footer: { text: "QUSM Ticket System • Final Approval Log" }, timestamp: new Date().toISOString() }], allowed_mentions: { users, roles: [LOGISTICS_ROLE_ID] } }) });
}
...
export async function postRejectionLog(request: TicketRequest, reason: string, rejectedBy: string, rejectedByUsername: string) {
  ...
  return discordApi(`/channels/${TICKET_LOG_CHANNEL_ID}/messages`, { method: "POST", body: JSON.stringify({ content: `<@&${LOGISTICS_ROLE_ID}>`, embeds: [{ title: "Ticket Log Rejected", description: `<@${request.userId}> ticket log has been rejected by Logistics.`, color: 0xed4245, fields: [
  ...
  ], image: { url: request.proof }, footer: { text: "QUSM Ticket System • Final Rejection Log" }, timestamp: new Date().toISOString() }], allowed_mentions: { users, roles: [LOGISTICS_ROLE_ID] } }) });
}
