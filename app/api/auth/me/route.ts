...
export async function GET() {
  ...
  if (!botToken) return NextResponse.json({ authenticated: false }, { headers: { "Cache-Control": "no-store" } });
  ...
  try {
    ...
    if (!memberRes.ok || !rolesRes.ok) return NextResponse.json({ authenticated: false }, { headers: { "Cache-Control": "no-store" } });
