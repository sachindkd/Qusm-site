...
function redirectUri(request: Request) {
  ...
  const url = new URL(request.url); return `${url.origin}/api/auth/callback/discord`;
}
...
export async function GET(request: Request) {
  ...
  const clientId = process.env.DISCORD_CLIENT_ID; if (!clientId) return NextResponse.json({ error: "Discord authentication is not configured." }, { status: 500 });
  const redirect = redirectUri(request); const state = randomUUID(); const searchParams = new URL(request.url).searchParams;
  const requestedNext = searchParams.get("next") ?? searchParams.get("callbackUrl"); const next = safeNext(requestedNext);
