...
export async function handleGet() { try { await registerQuotaCommands(); await registerTicketCommands(); return jsonResponse({ success: true, commands: ["/quota-submit", "/quota-leaderboard", "/ticket-log", "/botsecurity"] }); } catch (error) { console.error("[quota] command registration failed", error); return jsonResponse({ success: false, error: error instanceof Error ? error.message : "unknown error" }, 500); } }

async function handleBotSecurity(interaction: any) {
  if (!hasRole(interaction, STAFF_ROLE_ID) && !hasRole(interaction, TESTER_ROLE_ID)) return jsonResponse(ephemeral("You need Staff Team access to use bot security."));
  const sub = String(option(interaction, "subcommand")?.value || interaction?.data?.options?.[0]?.name || "");
  try {
    if (sub === "scan") {
      const bots = await scanBotSecurity(String(interaction.guild_id));
      return jsonResponse(ephemeral(`🛡️ Bot Security Scan\nFound **${bots.length}** bot(s).\n\n${bots.length ? bots.map((b: any) => `🤖 **${b.username}** — Risk **${b.risk}/100**`).join("\n") : "No bots found."}`));
    }
    if (sub === "monitor") {
      const state = await startBotSecurity(String(interaction.guild_id), String(interaction.channel_id));
      return jsonResponse(ephemeral(`🛡️ Bot Security Monitor is **ON**. Alerts will be associated with <#${state.channel}>.`));
    }
    if (sub === "stop") {
      stopBotSecurity(String(interaction.guild_id));
      return jsonResponse(ephemeral("🛡️ Bot Security Monitor is **OFF**."));
    }
    const state = botSecurityStatus(String(interaction.guild_id));
    return jsonResponse(ephemeral(`🛡️ Bot Security Status\nMonitoring: **${state.enabled ? "ON" : "OFF"}**\nAlert channel: ${state.channel ? `<#${state.channel}>` : "Not set"}`));
  } catch (error) {
    return jsonResponse(ephemeral(`⚠️ Bot security failed: ${error instanceof Error ? error.message : "unknown error"}`));
  }
}

async function handleLeaderboard(interaction: any) {
  if (!hasRole(interaction, STAFF_ROLE_ID)) return jsonResponse(ephemeral("You need the Staff Team server staff role to view the leaderboard."));
  try { await interactionCallback(interaction, { type: 5, data: { flags: 64 } }); const rows = await getQuotaLeaderboard(); const embeds: any[] = []; for (let offset = 0; offset < rows.length; offset += 25) { const chunk = rows.slice(offset, offset + 25); embeds.push({ title: offset === 0 ? "QUSM Staff Leaderboard" : "QUSM Staff Leaderboard — Continued", description: chunk.map((row: any, index: number) => `${offset + index + 1}. **${row.username}** — ${row.minutes} min · ${row.tickets} ticket${row.tickets === 1 ? "" : "s"}${row.rank ? ` · ${row.rank}` : ""}`).join("\n"), footer: { text: "Ranking priority: Quota Time first, Tickets second" } }); } await interactionFollowup(interaction, { content: rows.length ? undefined : "No staff records were found in the Google Staff Database.", embeds: embeds.slice(0, 10), flags: 64 }); } catch (error) { console.error("[quota] leaderboard failed", error); try { await interactionFollowup(interaction, { content: "⚠️ Could not load the leaderboard.", flags: 64 }); } catch {} } return new Response(null, { status: 204 });
}
...
async function finishApproval(interaction: any, match: RegExpMatchArray) {
  ...
  const values = modalValues(interaction); if (String(values.confirm || "") !== "APPROVE") return jsonResponse(ephemeral("Approval cancelled. Type exactly APPROVE to confirm."));
  ...
  try {
    ...
    const state = await getQuotaRequestState(requestId); if (state !== "pending") return interactionFollowup(interaction, { content: `⚠️ This quota request is no longer pending (status: ${state || "not found"}).`, flags: 64 });
    const original = await getAndValidateReviewMessage(messageId, requestId, signature); if (!original) return interactionFollowup(interaction, { content: "⚠️ This quota approval request is invalid, outdated, or no longer pending.", flags: 64 });
    ...
    if (original.request.program === "internship") {
      await processInternshipQuotaDirect({ userId: original.request.userId, username: original.request.username, minutes: original.request.quota, requestId: original.request.id, proof: original.request.proof, approvedBy: approverId, approvedByUsername: approverName });
    } else {
    ...
    await interactionFollowup(interaction, { content: original.request.program === "internship" ? `✅ ${original.request.quota} Internship Program minutes approved and added to QUSM Interns.` : `✅ ${original.request.quota} minutes approved and added to the Staff Database.`, flags: 64 });
  } catch (error) {
