...
export async function GET() {
  ...
  try {
    ...
    const res = await fetch(`https://discord.com/api/guilds/${FBMRP_GUILD_ID}/members/${session.id}`, { headers: { Authorization: `Bot ${botToken}` }, cache: "no-store" });
    if (!res.ok) return NextResponse.json({ authenticated: false });
    const member = await res.json();
    const access = getAccessLevel(session.id, member.roles || []);
    return NextResponse.json({ authenticated: true, user: { ...session, roles: member.roles || [], access } });
