...
export default function AdminClient({ initialContent, email, access, permissions }: { initialContent: any; email: string; access: string; permissions: string[] }) {
  ...
  return <div className="min-h-screen bg-bg text-white pb-28">
    <header className="border-b border-border px-6 sm:px-16 py-6 flex justify-between items-center">
      ...
      <button onClick={() => signOut({ callbackUrl: "/" })} className="font-mono text-[10px] uppercase border border-border px-3 py-2 flex gap-2 items-center"><LogOut size={12}/> Sign out</button>
    </header>
    <main className="px-6 sm:px-16 py-10 max-w-5xl mx-auto">
...
function FieldInput({ field, value, onChange }: { field: Field; value: any; onChange: (v: string) => void }) {
  const common = "mt-1 w-full bg-panel2 border border-border rounded-lg px-3 py-2.5 text-sm text-white outline-none focus:border-golddim";
  if (field.type === "textarea") return <textarea value={value ?? ""} onChange={e => onChange(e.target.value)} rows={4} className={common} />;
  if (field.type === "select") return <select value={value ?? field.options?.[0] ?? ""} onChange={e => onChange(e.target.value)} className={common}>{field.options?.map(o => <option key={o} value={o}>{o}</option>)}</select>;
  return <input type={field.type === "url" ? "url" : field.type === "date" ? "date" : "text"} value={value ?? ""} onChange={e => onChange(e.target.value)} className={common} />;
}

function CollectionEditor({ fields, items, setItems }: { fields: Field[]; items: any[]; setItems: (v: any[]) => void }) {
...
function SettingsEditor({ fields, value, setValue }: { fields: Field[]; value: any; setValue: (v: any) => void }) {
  return <section><div className="mb-5"><h3 className="font-serif text-2xl font-bold">Site settings</h3><p className="text-textfaint text-xs mt-1">Core organization information.</p></div><div className="border border-border bg-panel p-5 rounded-xl grid md:grid-cols-2 gap-4">{fields.map(field => <label key={field.key} className="font-mono text-[9px] uppercase text-textfaint">{field.label}<FieldInput field={field} value={value[field.key]} onChange={v => setValue({ ...value, [field.key]: v })} /></label>)}</div></section>;
}
