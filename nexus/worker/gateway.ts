...
export class NexusGatewayWorker {
  ...
  private async handle(payload: GatewayPayload, token: string) {
    ...
    if (payload.op === 10) {
      ...
      this.send({ op: 2, d: { token, intents: 1 | 512 | 32768 | 2 | 256, properties: { os: 'linux', browser: 'nexus', device: 'nexus' } } });
      return;
    }
    ...
    if (this.signals.length >= 5) await this.flushSecurity();
  }
  ...
  private async flushSecurity() {
    if (this.processing || this.signals.length < 5) return;
    this.processing = true;
    const batch = this.signals.slice();
    this.signals = [];
    try {
      const report = await investigateAndRespond(NEXUS_ALLOWED_GUILD_ID, batch);
      if (report.assessment.severity !== 'low') console.log('[NEXUS security incident]', JSON.stringify(report));
      void discordRequest;
