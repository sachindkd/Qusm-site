...
export async function GET(request: Request) {
  ...
  const tokenRes = await fetch("https://discord.com/api/oauth2/token", { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ client_id: clientId, client_secret: clientSecret, grant_type: "authorization_code", code, redirect_uri: redirectUri }) });
  ...
  const access = getAccessLevel(user.id, member.roles || []);
  const response = NextResponse.redirect(new URL("/staff", url.origin));
  response.cookies.set("fbmrp_discord_user", JSON.stringify({ id: user.id, username: user.username, avatar: user.avatar, roles: member.roles || [], access }), { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/", maxAge: 60 * 60 * 24 * 7 });
  return response;
}
