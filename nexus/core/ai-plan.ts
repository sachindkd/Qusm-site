...
export function planningPrompt(goal: string, guildId: string, conversation = ''): string {
  ...
  return [
    ...
    'Use outputs from earlier steps through SIMPLE DATA REFERENCES ONLY, such as {{stepId.result.id}} or {{stepId.result.0.id}}.',
    'Reference syntax is NOT JavaScript and MUST NOT contain .find(...), arrow functions, filters, expressions, comparisons, method calls, or other executable code.',
    'When inspect_roles returns an array, reference a specific returned role by a deterministic array index only when the index is established by inspection. Otherwise create a dedicated inspection/planning step whose output identifies the target ID using supported runtime data.',
    'Never emit references such as {{inspectRoles.result.roles.find(r => r.name === \'Nexus Admin\').id}}. The executor does not evaluate arbitrary expressions.',
