...
async function sendQuotaReview(request: QuotaRequest) { const sig = requestSignature(request).slice(0, 24); return discordApi(`/channels/${QUOTA_CHANNEL_ID}/messages`, { method: "POST", body: JSON.stringify({ embeds: [{ title: "Quota Submission — Pending Review", description: `<@${request.userId}> submitted a quota for Logistics review.`, fields: [{ name: "Request ID", value: request.id, inline: false }, { name: "Member", value: `<@${request.userId}> (${request.username})`, inline: true }, { name: "Quota (minutes)", value: `${request.quota} min`, inline: true }, { name: "Proof", value: request.proof, inline: false }, { name: "Notes", value: request.notes || "—", inline: false }], footer: { text: "QUSM Quota System • Staff Team" }, timestamp: request.createdAt }], components: [{ type: 1, components: [{ type: 2, style: 3, label: "Approve & Add Minutes", custom_id: `quota:approve:${request.id}:${sig}` }, { type: 2, style: 4, label: "Reject", custom_id: `quota:reject:${request.id}:${sig}` }] }], allowed_mentions: { users: [request.userId] } }) }); }
function extractRequest(interaction: any, signature: string): QuotaRequest | null { const fields = interaction?.message?.embeds?.[0]?.fields; if (!Array.isArray(fields)) return null; const values = Object.fromEntries(fields.map((field: any) => [String(field.name), String(field.value || "")])); const id = values["Request ID"]; const member = values["Member"] || ""; const quota = Number(String(values["Quota (minutes)"] || "").replace(/\s*min(?:utes?)?\s*$/i, "")); const proof = values["Proof"] || ""; const notes = values["Notes"] === "—" ? "" : (values["Notes"] || ""); const mention = member.match(/^<@(\d+)>/); if (!id || !mention?.[1] || !Number.isFinite(quota) || quota < 0 || !/^https?:\/\//i.test(proof)) return null; const request: QuotaRequest = { id, userId: mention[1], username: member.replace(/^<@\d+>\s*\((.*)\)$/, "$1"), quota, proof, notes, createdAt: String(interaction?.message?.timestamp || new Date().toISOString()) }; return requestSignature(request).slice(0, 24) === signature ? request : null; }
...
export async function POST(request: Request) {
  ...
  if (interaction.type === 5 && interaction.data?.custom_id === "quota_submit") { if (!hasRole(interaction, STAFF_ROLE_ID)) return json(ephemeral("You need the Staff role to submit quota.")); const values = modalValues(interaction); const quota = Number(values.quota); const proof = values.proof.trim(); const notes = values.notes.trim(); if (!Number.isFinite(quota) || quota < 0 || quota > 100000 || !/^https?:\/\//i.test(proof)) return json(ephemeral("Invalid quota minutes or proof URL.")); const user = interaction.member?.user; const quotaRequest: QuotaRequest = { id: randomUUID(), userId: user.id, username: user.global_name || user.username || user.id, quota, proof, notes, createdAt: new Date().toISOString() }; await sendQuotaReview(quotaRequest); return json(ephemeral(`✅ ${quota} minute(s) submitted. It is now waiting for Logistics approval.`)); }
  if (interaction.type === 3 && typeof interaction.data?.custom_id === "string") {
    ...
    try {
      if (action === "approve") await processQuotaDirect({ userId: quotaRequest.userId, username: quotaRequest.username, minutes: quotaRequest.quota, requestId: quotaRequest.id, proof: quotaRequest.proof, approvedBy: interaction.member.user.id, approvedByUsername: interaction.member.user.username });
