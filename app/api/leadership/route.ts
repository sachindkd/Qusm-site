...
async function ok() {
  ...
  try {
    ...
    return can(getAccessLevel(session.id, member.roles || [], roles as DiscordGuildRole[]), "leadership:edit");
  } catch {
    return false;
  }
}
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    const content = await readContent();
    const next = (content.leadership || []).filter((item) => item.id !== id);
    if (next.length === (content.leadership || []).length) return NextResponse.json({ error: "Not found" }, { status: 404 });
    await persistSection("leadership", next);
