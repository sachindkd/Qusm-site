...
import { processQuotaWithAI } from "@/lib/quota-ai";
...
function requestSignature(request: QuotaRequest) {
  return hmac(JSON.stringify({ id: request.id, userId: request.userId, username: request.username, quota: request.quota, proof: request.proof, notes: request.notes }));
}
...
function verifyDiscordSignature(body: string, timestamp: string, signature: string) {
  try { return verify(null, encoder.encode(timestamp + body), publicKey(), Buffer.from(signature, "hex")); } catch { return false; }
}
...
function modalValues(interaction: any) {
  ...
  for (const row of interaction?.data?.components || []) for (const component of row?.components || []) out[component.custom_id] = String(component.value || "");
  return out;
}
...
async function ensureQuotaCommandRegistered() {
  ...
  const payload = { name: "quota", description: "Submit your staff quota in minutes for Logistics review", type: 1, options: [] };
...
async function sendQuotaReview(request: QuotaRequest) {
  ...
  return discordApi(`/channels/${QUOTA_CHANNEL_ID}/messages`, {
    method: "POST",
    body: JSON.stringify({
      embeds: [{ title: "Quota Submission — Pending Review", description: `<@${request.userId}> submitted a quota for Logistics review.`, fields: [
      ...
      ], footer: { text: "QUSM Quota System • Staff Team" }, timestamp: request.createdAt }],
...
function extractRequest(interaction: any, signature: string): QuotaRequest | null {
  ...
  return requestSignature(request).slice(0, 24) === signature ? request : null;
}
...
export async function GET() {
  try { await ensureQuotaCommandRegistered(); return json({ success: true, command: "quota", guildId: STAFF_GUILD_ID }); }
  catch (error) { console.error("Failed to register /quota command from GET", error); return json({ success: false, error: error instanceof Error ? error.message : "unknown error" }, 500); }
}
...
export async function POST(request: Request) {
  ...
  if (interaction.type === 3 && typeof interaction.data?.custom_id === "string") {
    ...
    } else return json(ephemeral("Discord application ID is not configured."));

    try {
      if (action === "approve") {
        const result = await processQuotaWithAI({
          userId: quotaRequest.userId,
          username: quotaRequest.username,
          minutes: quotaRequest.quota,
          requestId: quotaRequest.id,
          proof: quotaRequest.proof,
          approvedBy: interaction.member.user.id,
        });
        console.log("Quota AI agent completed", result);
      }

      await discordApi(`/channels/${QUOTA_CHANNEL_ID}/messages/${interaction.message.id}`, {
        method: "PATCH",
        body: JSON.stringify({
          embeds: [{ ...interaction.message.embeds[0], title: action === "approve" ? "Quota Submission — Approved" : "Quota Submission — Rejected", footer: { text: `QUSM Quota System • ${action === "approve" ? "AI verified and minutes added to Google database" : "Rejected by Logistics"}` } }],
          components: [{ type: 1, components: [
            { type: 2, style: 3, label: action === "approve" ? "Approved & AI Added" : "Approved", custom_id: `quota:done:${requestId}`, disabled: true },
            { type: 2, style: 4, label: "Rejected", custom_id: `quota:done-reject:${requestId}`, disabled: true },
          ]}],
        }),
      });
      await followup(interaction, action === "approve" ? `✅ **${quotaRequest.username}** — ${quotaRequest.quota} minute(s) approved and processed by the QUSM AI agent.` : `❌ **${quotaRequest.username}** quota rejected.`);
    } catch (error) {
      console.error("Quota AI action failed", error);
