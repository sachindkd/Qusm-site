...
import { createDynamicPlan, generateNexusReply } from '../../../../core/ai-runtime';
...
function isCasualConversation(text: string): boolean {
  return /^(hi|hello|hey|yo|hiya|sup|how are you|good morning|good afternoon|good evening|thanks|thank you|ok|okay|cool|nice|great)[!. ]*$/i.test(text.trim());
}
...
async function runObjective(applicationId: string, token: string, goal: string, guildId: string, userId: string, channelId: string) {
  try {
    ...
    try { await followup(applicationId, token, errorLabel(error)); }
    catch (followupError) { console.error('NEXUS follow-up failed:', followupError); }
  }
}
...
async function runConversation(applicationId: string, token: string, message: string, guildId: string, userId: string, channelId: string) {
  try { await followup(applicationId, token, await generateNexusReply(message, guildId, userId, channelId)); }
    ...
    try { await followup(applicationId, token, errorLabel(error)); }
    catch (followupError) { console.error('NEXUS conversation follow-up failed:', followupError); }
  }
}
...
export async function POST(req: Request) {
  ...
  catch (error) { return NextResponse.json({ type: 4, data: { content: errorLabel(error) } }); }
  ...
  after(async () => isCasualConversation(goal)
    ? runConversation(applicationId, token, goal, guildId, userId, channelId)
    : runObjective(applicationId, token, goal, guildId, userId, channelId));
