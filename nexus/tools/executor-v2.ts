...
const ADVANCED = new Set(['inspect_channel', 'inspect_roles', 'create_category', 'create_embed', 'inspect_webhooks', 'create_webhook']);
export async function executeCapabilityV2(context: NexusServerContext, capability: string, input: Record<string, unknown>): Promise<ToolExecutionResult> {
  if (context.guildId !== process.env.NEXUS_ALLOWED_GUILD_ID && context.guildId !== '1549377442240663572') return { ok: false, capability, error: 'Authorization error: unauthorized guild.' };
  try {
    ...
    if (!ADVANCED.has(capability)) {
      if (['edit_channel', 'delete_channel', 'send_message', 'create_embed', 'inspect_webhooks', 'create_webhook'].includes(capability) && input.channelId) await discord.channel(context.guildId, String(input.channelId));
      if (['edit_role'].includes(capability) && input.roleId) { const roles: any[] = await discord.roles(context.guildId) as any[]; if (!roles.some((r) => String(r?.id) === String(input.roleId))) throw new Error('Authorization error: target role does not belong to the authorized guild.'); }
      if (['assign_role'].includes(capability) && input.roleId) { const roles: any[] = await discord.roles(context.guildId) as any[]; if (!roles.some((r) => String(r?.id) === String(input.roleId))) throw new Error('Authorization error: target role does not belong to the authorized guild.'); }
      if (['assign_role', 'timeout_member', 'kick_member', 'ban_member'].includes(capability) && input.userId) await discord.member(context.guildId, String(input.userId));
      return executeCapability(context, capability, input);
    }
    let result: unknown;
    switch (capability) {
