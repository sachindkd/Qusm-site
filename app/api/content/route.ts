...
async function getIdentity(): Promise<Identity> {
  ...
  if (!session) return { access: "member", userId: "", roleIds: [], username: "Unknown" };
  ...
  if (!token) return { access: "member", userId: session.id, roleIds: [], username: session.username || session.id };
  try {
    ...
    if (!memberResponse.ok || !rolesResponse.ok) return { access: "member", userId: session.id, roleIds: [], username: session.username || session.id };
    const member = await memberResponse.json() as { roles?: string[]; user?: { username?: string } };
    const roles = await rolesResponse.json() as DiscordGuildRole[];
    const roleIds = member.roles ?? [];
    return { access: getAccessLevel(session.id, roleIds, roles), userId: session.id, roleIds, username: member.user?.username || session.username || session.id };
  } catch {
    return { access: "member", userId: session.id, roleIds: [], username: session.username || session.id };
  }
}
