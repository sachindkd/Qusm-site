...
export const NEXUS_CAPABILITIES: NexusCapability[] = [
  ...
  { name: 'inspect_member', description: 'Inspect one member in the authorized guild.', inputSchema: { guildId: 'string', userId: 'string' }, risk: 'read' },
  ...
  { name: 'create_channel', description: 'Create a supported Discord channel in the authorized guild.', inputSchema: { guildId: 'string', name: 'string', type: 'number?', parentId: 'string?', topic: 'string?', reason: 'string?' }, risk: 'change' },
  { name: 'edit_channel', description: 'Edit a channel in the authorized guild, including permission overwrites.', inputSchema: { guildId: 'string', channelId: 'string', changes: 'object', reason: 'string?' }, risk: 'change' },
  ...
  { name: 'inspect_webhooks', description: 'Inspect webhooks for a channel in the authorized guild.', inputSchema: { guildId: 'string', channelId: 'string' }, risk: 'read' },
  { name: 'create_webhook', description: 'Create a webhook for a channel when useful for the objective.', inputSchema: { guildId: 'string', channelId: 'string', name: 'string', avatar: 'string?', reason: 'string?' }, risk: 'change' },
