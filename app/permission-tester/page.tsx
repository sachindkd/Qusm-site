...
import { ROLE_IDS, getAccessLevel, getPermissions, type AccessLevel } from "@/lib/discord-roles";

const TEST_ROLES = [
  ["Owner", ROLE_IDS.owner, "owner"],
  ["Co-Owner", ROLE_IDS.coOwner, "owner"],
  ["Chairman", ROLE_IDS.chairman, "owner"],
  ["Vice Chairman", ROLE_IDS.viceChairman, "owner"],
  ["OFC Admin", ROLE_IDS.ofcAdmin, "owner"],
  ["Head Operations", ROLE_IDS.headOperations, "ownership"],
  ["Head Administration", ROLE_IDS.headAdministration, "ownership"],
  ["Community Affairs", ROLE_IDS.communityAffairs, "ownership"],
  ["CEO", ROLE_IDS.ceo, "ownership"],
  ["Head Development", ROLE_IDS.headDevelopment, "ownership"],
  ["Head Management", ROLE_IDS.headManagement, "senior-leadership"],
  ["General Manager", ROLE_IDS.generalManager, "senior-leadership"],
  ["Head Department", ROLE_IDS.headDepartment, "senior-leadership"],
  ["Senior Management", ROLE_IDS.seniorManagement, "senior-leadership"],
  ["Developer Posts", ROLE_IDS.developerPosts, "developer"],
  ["Aides", ROLE_IDS.aides, "aide"],
  ["Staff", ROLE_IDS.staff, "staff"],
...
export default async function PermissionTester({ searchParams }: { searchParams: Promise<{ role?: string }> }) {
  ...
  if (discordId !== "1210317929485181000") redirect("/live");
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="grid gap-6 lg:grid-cols-[360px_1fr]">
          <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
            ...
            <p className="mt-1 text-xs text-white/40">Select any configured role to test its website access.</p>
            <div className="mt-4 space-y-2">
              {TEST_ROLES.map(([name, id]) => (
                <a key={name} href={id ? `/permission-tester?role=${encodeURIComponent(id)}` : "/permission-tester"} className={`block rounded-xl border px-4 py-3 text-sm transition ${name === selectedName ? "border-amber-300/40 bg-amber-300/10 text-amber-200" : "border-white/10 bg-black/10 text-white/75 hover:bg-white/[0.06]"}`}>
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-6">
              ...
              <div className="mt-3 flex flex-wrap items-end justify-between gap-4">
                <div><h2 className="text-2xl font-bold">{selectedName}</h2><p className="mt-1 font-mono text-xs text-white/35">Role ID: {selectedRoleId || "none"}</p></div>
