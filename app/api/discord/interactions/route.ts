...
const WEBSITE_LEADERBOARD_URL = "https://qusm-sitev1.vercel.app/admin";
...
async function ensureQuotaCommandRegistered() {
  ...
  const payload = { name: "quota", description: "Submit or view staff quota", type: 1, options: [
    { type: 1, name: "submit", description: "Submit quota with a proof image for Logistics review", options: [
    ...
    { type: 1, name: "leaderboard", description: "View the live quota leaderboard from the staff database", options: [] },
  ] };
  if (existing?.id) await discordApi(`/applications/${APPLICATION_ID}/guilds/${STAFF_GUILD_ID}/commands/${existing.id}`, { method: "PATCH", body: JSON.stringify(payload) }); else await discordApi(`/applications/${APPLICATION_ID}/guilds/${STAFF_GUILD_ID}/commands`, { method: "POST", body: JSON.stringify(payload) });
}
...
async function sendQuotaReview(request: QuotaRequest, displayName: string) { const sig = requestSignature(request).slice(0, 24); return discordApi(`/channels/${QUOTA_CHANNEL_ID}/messages`, { method: "POST", body: JSON.stringify({ embeds: [{ title: "Quota Submission — Pending Review", description: `<@${request.userId}> submitted a quota for Logistics review.`, fields: [{ name: "Request ID", value: request.id, inline: false }, { name: "Member", value: `<@${request.userId}> (${request.username})`, inline: true }, { name: "Display Name", value: displayName || request.username, inline: true }, { name: "Quota (minutes)", value: `${request.quota} min`, inline: true }, { name: "Proof", value: `[${request.proofName || "View proof image"}](${request.proof})`, inline: false }, { name: "Notes", value: request.notes || "—", inline: false }], image: { url: request.proof }, footer: { text: "QUSM Quota System • Staff Team" }, timestamp: request.createdAt }], components: [{ type: 1, components: [{ type: 2, style: 3, label: "Approve & Add Minutes", custom_id: `quota:approve:${request.id}:${sig}` }, { type: 2, style: 4, label: "Reject", custom_id: `quota:reject:${request.id}:${sig}` }] }], allowed_mentions: { users: [request.userId] } }) }); }

async function sendQuotaApprovalLog(request: QuotaRequest, approvedById: string, approvedByUsername: string) { const allowedUsers = [...new Set([request.userId, approvedById])]; return discordApi(`/channels/${QUOTA_LOG_CHANNEL_ID}/messages`, { method: "POST", body: JSON.stringify({ embeds: [{ title: "Quota Approved", description: `<@${request.userId}> quota has been approved and added to the Google Staff Database.`, color: 0x57f287, fields: [{ name: "Staff Member", value: `<@${request.userId}> (${request.username})`, inline: true }, { name: "Minutes Added", value: `${request.quota} min`, inline: true }, { name: "Approved By", value: `<@${approvedById}> (${approvedByUsername})`, inline: true }, { name: "Request ID", value: request.id, inline: false }, { name: "Proof", value: `[${request.proofName || "View proof image"}](${request.proof})`, inline: false }, { name: "Notes", value: request.notes || "—", inline: false }], image: { url: request.proof }, footer: { text: "QUSM Quota System • Approval Log" }, timestamp: new Date().toISOString() }], allowed_mentions: { users: allowedUsers } }) }); }
...
async function sendQuotaLeaderboard(interaction: any) { const rows = await getQuotaLeaderboard(); if (!rows.length) return followup(interaction, "No staff quota records were found in the Google Staff Database."); const chunks: any[] = []; for (let i = 0; i < rows.length; i += 25) { const chunk = rows.slice(i, i + 25); chunks.push({ title: i === 0 ? "QUSM Quota Leaderboard" : "QUSM Quota Leaderboard — Continued", color: 0xd4af37, description: chunk.map((row, index) => `${i + index + 1}. **${row.username}** — ${row.minutes} min${row.rank ? ` · ${row.rank}` : ""}`).join("\n"), footer: { text: "Live from QUSM Staff Database • Column E" } }); } await fetch(`https://discord.com/api/v10/webhooks/${APPLICATION_ID}/${interaction.token}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ embeds: chunks.slice(0, 10), flags: 64 }), cache: "no-store" }); }
...
export async function POST(request: Request) {
  ...
  if (interaction.type === 2 && interaction.data?.name === "quota") {
    ...
    if (command === "leaderboard") {
      if (!hasRole(interaction, WEBSITE_STAFF_ROLE_ID)) return json(ephemeral("You need the website Staff role to view the quota leaderboard."));
      return json({ type: 4, data: { content: `**Live Quota Leaderboard**\n${WEBSITE_LEADERBOARD_URL}\n\nWould you like me to fetch the live information here?`, flags: 64, components: [{ type: 1, components: [{ type: 2, style: 3, label: "Yes — Fetch Here", custom_id: "quota:leaderboard:yes" }, { type: 2, style: 2, label: "No — Use Website", custom_id: "quota:leaderboard:no" }] }] } });
    }
    if (command !== "submit") return json(ephemeral("Use `/quota submit` to submit quota or `/quota leaderboard` to view the leaderboard."));
  ...
  if (interaction.type === 3 && typeof interaction.data?.custom_id === "string") {
    ...
    if (customId === "quota:leaderboard:yes") {
      if (!hasRole(interaction, WEBSITE_STAFF_ROLE_ID)) return json(ephemeral("You need the website Staff role to view the quota leaderboard."));
      ...
      try { await sendQuotaLeaderboard(interaction); } catch (error) { console.error("Quota leaderboard fetch failed", error); await followup(interaction, `⚠️ Could not fetch the live leaderboard: ${error instanceof Error ? error.message : "unknown error"}`); }
      return new Response(null, { status: 204 });
    }
    if (customId === "quota:leaderboard:no") return json(ephemeral(`No problem — use the Staff Management leaderboard here: ${WEBSITE_LEADERBOARD_URL}`));
    ...
    const match = customId.match(/^quota:(approve|reject):([^:]+):([a-f0-9]{24})$/); if (!match) return json(ephemeral("Unknown quota action.")); const [, action, requestId, signature] = match;
    ...
    try {
      ...
      await followup(interaction, `✅ **${quotaRequest.username}** — ${quotaRequest.quota} minute(s) approved and added directly to the Google Staff Database.`);
  ...
  if (interaction.type === 5 && typeof interaction.data?.custom_id === "string" && interaction.data.custom_id.startsWith("quota_reject:")) {
    const match = interaction.data.custom_id.match(/^quota_reject:([^:]+):([a-f0-9]{24}):(\d+)$/); if (!match) return json(ephemeral("Invalid rejection form."));
    if (!(hasRole(interaction, LOGISTICS_ROLE_ID) || hasRole(interaction, TESTER_ROLE_ID))) return json(ephemeral("Only Logistics can reject quota submissions."));
    if (!APPLICATION_ID) return json(ephemeral("Discord application ID is not configured."));
    const [, requestId, signature, messageId] = match; const message = await discordApi(`/channels/${QUOTA_CHANNEL_ID}/messages/${messageId}`, { method: "GET" });
    const quotaRequest = extractRequest({ message }, signature); if (!quotaRequest || quotaRequest.id !== requestId) return json(ephemeral("This quota request is invalid or has been tampered with."));
    const reason = modalValues(interaction).reason?.trim(); if (!reason) return json(ephemeral("A rejection reason is required."));
    await fetch(`https://discord.com/api/v10/interactions/${interaction.id}/${interaction.token}/callback`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: 5, data: { flags: 64 } }), cache: "no-store" });
    try {
      ...
      await followup(interaction, `❌ **${quotaRequest.username}** quota rejected. The requester was sent the reason by DM.`);
    } catch (error) { console.error("Quota rejection failed", error); await followup(interaction, `⚠️ Rejection action failed: ${error instanceof Error ? error.message : "unknown error"}`); }
    return new Response(null, { status: 204 });
  }
