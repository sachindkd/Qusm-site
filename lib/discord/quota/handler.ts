...
async function handleButton(interaction: any) {
  ...
  const approve = customId.match(/^quota:approve:([0-9a-f-]{36}):([0-9a-f]{24}):([0-9]{10,25})$/i);
  const reject = customId.match(/^quota:reject:([0-9a-f-]{36}):([0-9a-f]{24}):([0-9]{10,25})$/i);
  ...
  return jsonResponse(approve ? approveModal(match[1], match[2].toLowerCase(), match[3]) : rejectModal(match[1], match[2].toLowerCase(), match[3]));
}
