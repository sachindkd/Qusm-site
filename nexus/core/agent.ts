...
function isSameObjective(a: string | undefined, b: string): boolean { return Boolean(a && b && a.trim().toLowerCase() === b.trim().toLowerCase()); }
...
export async function runAgentTurn(message: string, context: NexusContext, forcedGoal?: string): Promise<AgentTurnResult> {
  ...
  const userResponse = await generateExecutionResponse({
  ...
  remember(context, 'assistant', userResponse);
