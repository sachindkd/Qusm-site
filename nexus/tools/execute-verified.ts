...
export async function executeAndVerify(context: NexusServerContext, capability: string, input: Record<string, unknown>): Promise<ToolExecutionResult & { verification?: ToolExecutionResult }> {
  ...
  else if (capability === 'assign_role' || capability === 'timeout_member' || capability === 'kick_member' || capability === 'ban_member') verification = await executeCapabilityV2(context, 'inspect_members', { guildId: context.guildId, query: String(input.userId || ''), limit: 5 });
