...
export async function handlePost(interaction: any) { if (interaction.type === 1) return jsonResponse({ type: 1 }); if (interaction.guild_id !== STAFF_GUILD_ID) return jsonResponse(ephemeral("This quota system is only available in the Staff Team server.")); if (interaction.type === 2 && interaction.data?.name === "ask-ai") return handleAskAi(interaction); if (interaction.type === 2 && interaction.data?.name === "staff-performance") return handlePerformanceReport(interaction, "staff");
