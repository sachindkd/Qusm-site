...
export async function writeDbSection(section:string,value:unknown){await initDb();if(!/^[a-zA-Z0-9_]+$/.test(section))throw new Error("Invalid content section");const q=sql();const rows=await q`UPDATE site_content SET data=jsonb_set(data,ARRAY[${section}]::text[],${JSON.stringify(value)}::jsonb,true),updated_at=NOW() WHERE id=1 RETURNING data`;if(!rows.length)throw new Error("Content record does not exist");return rows[0].data as Content}
export async function dbHealth(){const q=sql();const rows=await q`SELECT 1 AS ok`;return rows[0]?.ok===1}
