...
function readCookie(request: Request, name: string) { return request.headers.get("cookie")?.match(new RegExp(`(?:^|;\\s*)${name}=([^;]+)`))?.[1]; }
function safeDestination(value: string | null) {
  if (!value || !value.startsWith("/") || value.startsWith("//")) return "/member";
  const allowed = ["/member", "/live", "/staff", "/admin", "/admin/builder", "/permission-tester", "/quota"];
...
export async function GET(request: Request) {
  ...
  const clientId = process.env.DISCORD_CLIENT_ID; const clientSecret = process.env.DISCORD_CLIENT_SECRET; const botToken = process.env.DISCORD_BOT_TOKEN; const sessionSecret = process.env.NEXTAUTH_SECRET;
  ...
  const token = await tokenRes.json(); if (!token?.access_token || typeof token.access_token !== "string") return NextResponse.redirect(new URL("/?auth=token_error", url.origin));
  ...
  const user = await meRes.json(); if (!user?.id || typeof user.id !== "string") return NextResponse.redirect(new URL("/?auth=user_error", url.origin));
  ...
  const member = await memberRes.json(); const roles = Array.isArray(member.roles) ? member.roles.filter((id: unknown): id is string => typeof id === "string") : [];
  const access = getAccessLevel(user.id, roles); const nextCookie = readCookie(request, OAUTH_NEXT_COOKIE); const requested = safeDestination(nextCookie ? decodeURIComponent(nextCookie) : null);
  const privileged = access !== "member" || requested === "/quota";
  const destination = privileged && ["/staff", "/admin", "/admin/builder", "/permission-tester", "/quota"].includes(requested) ? requested : "/member";
